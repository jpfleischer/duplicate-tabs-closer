...
const toggleExpendGroup = (groupId, checkbox, resize) => {
	if (checkbox) {
		...
		if (
			(thumbChecked && $(`#${listGroupId}`).hasClass("list-group-collapsed")) ||
			(!thumbChecked && $(`#${listGroupId}`).hasClass("list-group-expanded"))
		) {
			$(`#${listGroupId}`).toggleClass(
				"list-group-expanded list-group-collapsed",
			);
		}
		if (resize) resizeDuplicateTabsPanel();
	} else {
	...
	$(".list-group-item").toggleClass(
		"list-group-item-overflow",
		$(".card-body").css("height") >= $(".card-body").css("max-height"),
	);
};
	...
	if (storedOption === "environment" && value === "chrome") {
		...
		if (typeof value === "boolean") {
			...
			if (storedOption.endsWith("Pinned"))
				toggleExpendGroup(storedOption, true);
			...
			if (storedOption === "onDuplicateTabDetected")
				changeAutoCloseOptionState(value, resize);
		}
...
const handleMessage = (message) => {
	...
	if (message.action === "setStoredOption")
		setPanelOption({
